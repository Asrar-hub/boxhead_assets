function §\x01\x02§()
{
   return 1026;
}
var §\x01§ = -67 + "\x01\x02"();
var _loc2_;
while(true)
{
   if(eval("\x01") == 959)
   {
      set("\x01",eval("\x01") - 523);
      §§push(true);
   }
   else if(eval("\x01") == 847)
   {
      set("\x01",eval("\x01") + 12);
      §§push(eval(function §\x01\x02§()
      {
         return 1026;
      }));
   }
   else if(eval("\x01") == 551)
   {
      set("\x01",eval("\x01") + 391);
      if(function §\x01\x02§()
      {
         return 1026;
      })
      {
         set("\x01",eval("\x01") - 350);
      }
   }
   else if(eval("\x01") == 54)
   {
      set("\x01",eval("\x01") + 627);
   }
   else
   {
      if(eval("\x01") == 592)
      {
         set("\x01",eval("\x01") + 148);
         if(!eval("J4|")["{invalid_utf8=150}ن"])
         {
            eval("J4|")["{invalid_utf8=150}ن"] = new §\§\§constant(2)§();
         }
         function §\x01\x02§()
         {
            return 1026;
         }
         if(!eval("J4|")["{invalid_utf8=150}ن"][§§constant(3)])
         {
            eval("{invalid_utf8=150}ن")[§§constant(3)] extends eval("{invalid_utf8=150}ن")[§§constant(4)];
            _loc2_ = eval("{invalid_utf8=150}ن")[§§constant(3)] = function()
            {
               super();
            }[§§constant(5)];
            _loc2_[§§constant(6)] = function()
            {
               return this[§§constant(7)];
            };
            _loc2_[§§constant(8)] = function()
            {
               super[§§constant(8)]();
            };
            _loc2_[§§constant(9)] = function()
            {
               super[§§constant(9)]();
               if(this[§§constant(10)][§§constant(11)] && this[§§constant(12)]())
               {
                  this[§§constant(13)] = §§constant(14);
                  this[§§constant(15)] = false;
               }
            };
            _loc2_[§§constant(16)] = function()
            {
               super[§§constant(16)]();
            };
            _loc2_[§§constant(7)] = §§constant(3);
            _loc2_[§§constant(17)] = true;
            §§push(§§constant(18)(eval("{invalid_utf8=150}ن")[§§constant(3)][§§constant(5)],null,1));
         }
         §§pop();
         break;
      }
      if(eval("\x01") == 436)
      {
         set("\x01",eval("\x01") + 54);
         if(function §\x01\x02§()
         {
            return 1026;
         })
         {
            set("\x01",eval("\x01") + 421);
         }
      }
      else if(eval("\x01") == 859)
      {
         set("\x01",eval("\x01") - 308);
         §§push(!function §\x01\x02§()
         {
            return 1026;
         });
      }
      else if(eval("\x01") == 38)
      {
         set("\x01",eval("\x01") + 690);
      }
      else if(eval("\x01") == 42)
      {
         set("\x01",eval("\x01") + 805);
         §§push("\x0f");
      }
      else if(eval("\x01") == 846)
      {
         set("\x01",eval("\x01") - 63);
         §§push(true);
      }
      else if(eval("\x01") == 527)
      {
         set("\x01",eval("\x01") + 201);
      }
      else if(eval("\x01") == 728)
      {
         set("\x01",eval("\x01") - 619);
         §§push("\x0f");
         §§push(1);
      }
      else
      {
         if(eval("\x01") == 490)
         {
            set("\x01",eval("\x01") + 421);
            nextFrame();
            break;
         }
         if(eval("\x01") == 912)
         {
            set("\x01",eval("\x01") - 874);
            break;
         }
         if(eval("\x01") == 560)
         {
            set("\x01",eval("\x01") + 352);
            if(function §\x01\x02§()
            {
               return 1026;
            })
            {
               set("\x01",eval("\x01") - 874);
            }
         }
         else if(eval("\x01") == 109)
         {
            set("\x01",eval("\x01") - 67);
            var §§pop() = function §\x01\x02§()
            {
               return 1026;
            };
         }
         else if(eval("\x01") == 958)
         {
            set("\x01",eval("\x01") - 112);
         }
         else if(eval("\x01") == 942)
         {
            set("\x01",eval("\x01") - 350);
         }
         else
         {
            if(eval("\x01") == 740)
            {
               set("\x01",eval("\x01") - 740);
               break;
            }
            if(eval("\x01") == 911)
            {
               set("\x01",eval("\x01") - 230);
            }
            else if(eval("\x01") == 783)
            {
               set("\x01",eval("\x01") - 75);
               if(function §\x01\x02§()
               {
                  return 1026;
               })
               {
                  set("\x01",eval("\x01") - 137);
               }
            }
            else if(eval("\x01") == 569)
            {
               set("\x01",eval("\x01") - 9);
               §§push(true);
            }
            else if(eval("\x01") == 252)
            {
               set("\x01",eval("\x01") + 527);
               if(function §\x01\x02§()
               {
                  return 1026;
               })
               {
                  set("\x01",eval("\x01") - 111);
               }
            }
            else if(eval("\x01") == 668)
            {
               set("\x01",eval("\x01") + 178);
            }
            else if(eval("\x01") == 681)
            {
               set("\x01",eval("\x01") - 429);
               §§push(true);
            }
            else if(eval("\x01") == 426)
            {
               set("\x01",eval("\x01") + 143);
            }
            else
            {
               if(eval("\x01") == 779)
               {
                  set("\x01",eval("\x01") - 111);
                  break;
               }
               if(eval("\x01") != 571)
               {
                  if(eval("\x01") == 708)
                  {
                     set("\x01",eval("\x01") - 137);
                  }
                  break;
               }
               set("\x01",eval("\x01") - 2);
            }
         }
      }
   }
}
